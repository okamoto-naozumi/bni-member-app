...
1. **列が存在しない場合の自動フォールバック**(`src/lib/postgrestError.ts` の `extractMissingColumn`)
   ...
   **略歴・G.A.I.N.S.専用のローカルキャッシュ(`BIO_GAINS_CACHE_KEY`, `cacheBioGainsFields`/`mergeBioGainsCache`)**: 上記の列除外リトライは「保存自体を成功させる」ための仕組みだが、その結果Supabase側にbio_*/gains_*列が実際には保存されない(=次回`fetchMembers`で読み込むとその項目だけ空になる)ことは変わらない。これを補うため、`createMember`/`updateMember`は実際にSupabaseへ保存できたかに関わらず、送信した17項目の入力値を**常に**localStorageへキャッシュし、`normalizeMember`が返す全てのmember(`fetchMembers`/`fetchMemberById`/insert・update結果すべて)に対して「DB側の値が空ならキャッシュ値で補完し、DB側に値があればキャッシュを最新化する」形でマージする。これにより、本番DBが未マイグレーションで17列が存在しない間も、同じブラウザからは入力内容が「保存後に消えて未入力になる」ように見えない。**あくまで同一ブラウザ内の見かけ上の保護であり、`supabase/schema.sql`を実行して実DBに列を追加しない限り、別ブラウザ・別端末や新しいSupabaseクエリ結果には反映されない**点に注意(根本解決は必ずスキーマの手動マイグレーション)。
